...
class TestStat(APITestCase):
    def test_sendings(self):
        mail_count = Sending.objects.all().count()
        mail_create = {
            "date_start": now(),
            "date_end": now(),
            "time_start": now().time(),
        ...
        response = self.client.post("http://127.0.0.1:8000/api/sending/", mail_create)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Sending.objects.all().count(), mail_count + 1)
    ...
    def test_stat(self):
        self.test_sendings()
        url = "http://127.0.0.1:8000/api/sending"
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["Total number of sending"], 1)
        self.assertIsInstance(response.data["Total number of sending"], int)
